  ...
  modal.innerHTML = `
    <div class="modal-content" style="max-width: 500px;">
      ...
      <div class="modal-body">
        ...
        <button class="admin-btn admin-btn-primary" id="saveProductBtn" style="width: 100%;">Save Changes</button>
      </div>
    </div>
  `;
...
// Expose functions to global scope for inline onclick handlers (defined after closeProductModal)
...
async function parseAndExecuteSQL(query) {
  ...
  // Check for DDL commands first
  const isDDL = q.startsWith('create') || 
                q.startsWith('alter') || 
                q.startsWith('drop') || 
  ...
  if (isDDL) {
    const html = `
      <div style="background: #fef3c7; border-left: 4px solid #f59e0b; padding: 1rem; border-radius: 4px; margin-bottom: 1rem;">
        <strong style="color: #92400e; display: block; margin-bottom: 0.5rem;">⚠️ DDL Command Detected</strong>
        <p style="color: #78350f; margin: 0.5rem 0;">
          DDL commands (CREATE, ALTER, DROP, TRUNCATE, RENAME) cannot be executed through this admin panel.
        </p>
      </div>
      <div style="background: #dbeafe; border-left: 4px solid #3b82f6; padding: 1rem; border-radius: 4px;">
        <strong style="color: #1e40af; display: block; margin-bottom: 0.5rem;">📝 How to Execute Your DDL Command:</strong>
        <ol style="color: #1e3a8a; margin: 0.5rem 0; padding-left: 1.5rem;">
          ...
          <li>Select your project: <strong>iyddgoxxvygfavgoxzrf</strong></li>
          <li>Click on <strong>SQL Editor</strong> in the left sidebar</li>
          <li>Paste your DDL command in the editor</li>
          <li>Click <strong>Run</strong> to execute</li>
        </ol>
        <div style="background: white; padding: 0.75rem; border-radius: 4px; margin-top: 0.75rem; font-family: monospace; font-size: 0.9em; color: #1e293b; overflow-x: auto;">
          ${query}
        </div>
      </div>
    `;
  ...
  if (q.includes('from products') || q.includes('from adminproducts')) {
    ...
    throw new Error('Unknown table. Available tables: products, orders, customers, cart, wishlist');
  }
...
function describeTable(query) {
  ...
  const schemas = {
    ...
    customers: ['id', 'email', 'full_name', 'phone', 'dob', 'gender', 'address', 'city', 'state', 'pincode', 'country', 'created_at'],
  ...
  if (!cols) throw new Error(`Unknown table: ${table}. Available: products, orders, customers, cart, wishlist`);
...
function getColumnsForTable(table) {
  const columnMap = {
    ...
    customers: ['id', 'email', 'full_name', 'phone', 'dob', 'gender'],
...
// ============================================================================
// Make functions accessible for inline onclick handlers
...
window.viewSupabaseData = viewSupabaseData;
